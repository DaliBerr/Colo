        ...
        public void GenerateMap()
        {
            ...
            for (int z = 0; z < height; z++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    {
                        float w = Math.FBM((x + _offWarp.x) / warpScale, (z + _offWarp.y) / warpScale,
                        warpOctaves, 2f, 0.5f);
                        // 映射到 [-1,1]
                        w = w * 2f - 1f;
                        wx += w * warpAmplitude;
                    ...
                    // 2) 采样低频地势（0..1）
                    float elev = Math.FBM((wx + _offElev.x) / elevScale, (wz + _offElev.y) / elevScale,
                    ...
                    float rough = Math.FBM((wx + _offRough.x) / roughScale, (wz + _offRough.y) / roughScale,
